...
5. **Determine `target_platforms`:** Ask the user which platforms they want to generate for, or infer from context. Default to both (`antigravity`, `claude-code`) if not specified. Codex is available as opt-in via `ski-platform-exporter`. Include this as a field in the structured draft.
6. Present the structured draft to the user.
7. Apply corrections if the user requires them.
8. Return the consolidated document to the orchestrator as the **Step 0 handoff** for flow toward Discovery (Step 1), with `target_platforms` explicitly included.
...
**Pre-output — Compliance self-check:** Before returning the final handoff, emit a `<sys-eval>` block per `rul-strict-compliance`. List active Hard Constraints and verify all assigned Tasks have been executed. Only proceed after closing the tag.
