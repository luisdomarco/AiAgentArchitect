...
| --------------------------- | ------------------------------------------------ | ------------------------------------------------------- |
| `kno-entity-types` | `../knowledge-base/kno-entity-types.md` | Definition, structure and specifications of each entity type |
...
- Are there automated tasks that should run headlessly? (Script candidate)
- Are there system events that should trigger actions automatically? (Hook candidate)
...
Model strategy is implicitly `single-model` in Express Mode — do not specify model in entity frontmatter, user configures at session level.
...
**Step 5 — Model strategy selection**

Consult `kno-agent-strategies` §3 (Selection Guide) and evaluate which model composition strategy fits the system based on:

- Number and variety of agents (single agent → Single Model)
- Complexity distribution across intricacy levels assigned in Step 4
- Whether the system is planning-heavy (→ Orchestrator-Worker) or execution-heavy with occasional hard decisions (→ Advisor)
- Volume of generated outputs requiring quality gates (→ Evaluator-Gate)

If strategy = `advisor`:

- **Determine advisor count:** group executors by primary knowledge domain. If 2+ domains have 2+ executors each → multi-advisor (`age-spe-advisor-{domain}` per cluster). Otherwise → single advisor (`age-spe-advisor`).
- **Single advisor:** add `age-spe-advisor` (frontier / max / intricacy: medium) to the entity list.
- **Multi-advisor:** add `age-spe-advisor-{domain}` per domain (frontier / max / intricacy: medium). Set each executor's `escalation_domain` to its advisor's domain.
- Executor agents inherit escalation instructions (Report format, chat visibility, signals) and the `advisor-incidents/` directory protocol from `kno-agent-strategies` §2.4.

Record the selected strategy for inclusion in the Blueprint.

**Step 6 — Blueprint generation**

Use the Blueprint output template from **`../resources/res-architecture-designer-protocol.md` §1**. Includes the entity-count verification pre-step.

**Step 7 — Architecture diagram (Architect Mode)**

Activate `ski-diagram-generator` to produce the architecture diagram in Mermaid following the spec in **`../resources/res-architecture-designer-protocol.md` §2** (entities as nodes, labeled relationships, external systems differentiated, main data flow).

---

### 7.4 Building the handoff JSON

**Pre-output — Compliance self-check:** Before presenting the Blueprint or building the handoff JSON, emit a `<sys-eval>` block per `rul-strict-compliance`. List active Hard Constraints and verify all assigned Tasks have been executed. Only proceed after closing the tag.
...
- The creation order must respect dependencies: create first the entities that others reference (Scripts before Hooks, Skills and Rules before Agents, Agents before Workflows).
- When proposing Hooks, always identify the associated Script (if any). A Hook without a clear action is an incomplete design.
